...
//Aquí reside la lógica SQL compleja. Este archivo evita que el controlador se ensucie con consultas a la base de datos.
const { query } = require('../config/db'); // Importamos el query wrapper

class ProductDAO {

    /**
     * --- LA SUPER FUNCIÓN "SELECCIONAR" ---
     * Sirve para: Catálogo Cliente, Filtros Admin, Ofertas y Búsquedas.
    ...
    async seleccionar(filtros = {}) {
        ...
        // 1. Filtro Nombre (Búsqueda parcial e insensible a mayúsculas)
        if (filtros.nombre) {
        ...
        // 2. Filtro Talle (Busca en tabla hija Inventario)
        if (filtros.talle) {
        ...
        // 3. Filtro Etiqueta (Ej: 'navidad', 'oferta')
        if (filtros.tag) {
        ...
        // 4. Filtro Temporada (Lógica: Verano incluye Neutro)
        ...
        // --- PAGINACIÓN (Protección de memoria RAM) ---
    ...
    async create(data, imagenUrl) {
        ...
        const resProd = await query(sqlProd, [
            data.nombre, data.precio, genero, tipo, temporada, imagenUrl, data.descripcion, atributos
        ]);
    ...
    // Método getById reutilizando lógica
    async getById(id) {
        // Usamos una query directa optimizada para detalle
        const sql = `SELECT * FROM productos WHERE id = $1 AND deleted_at IS NULL`;
        const prodRes = await query(sql, [id]);
        
        if(!prodRes.rows[0]) return null;
        const prod = prodRes.rows[0];
        
        // Traer inventario detallado
